...
inline std::vector<int64_t> Model::get_operation_shape(
    ...
    if (n_dims > 0) {
        ...
        TF_GraphGetTensorShape(graph_.get(), out_op, dims, n_dims,
                               context::get_status());
...
inline std::vector<Tensor> Model::forward(
    ...
    for (int i = 0; i < inputs.size(); i++) {
        // Operations
        auto [op_name, op_idx] = parse_name(std::get<0>(inputs[i]));
        inp_ops[i].oper
            = TF_GraphOperationByName(graph_.get(), op_name.c_str());
    ...
    TF_SessionRun(session_.get(), NULL, inp_ops.data(), inp_val.data(),
                  inputs.size(), out_ops.data(), out_val.get(), outputs.size(),
